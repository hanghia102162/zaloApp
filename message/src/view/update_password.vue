<template>
  ...
  >
    ...
    >
      ...
      >
        <h1 class="font-medium text-2xl">Nhập mã xác thực</h1>
        ...
        <div class="w-full relative">
          ...
          <label
            ...
            class="absolute left-2 top-1/2 -translate-y-1/2 left-0 peer-not-placeholder-shown:top-[-10px] peer-not-placeholder-shown:text-sm peer-focus:top-[-10px] transition-all duration-500 ease-linear"
          >
            Xác nhận với mật khẩu mới khẩu
          </label>
        </div>
        <button
          type="button"
          @click="updatePasswpord"
...
const route = useRoute(); // thêm dòng này de lay id tu http

const password = ref("");
const confirmPassword = ref("");
const otp = ref("");
const id = route.params.id;
const updatePasswpord = async () => {
  if (password.value !== confirmPassword.value) {
    alert("password ko dung");
    return;
  }
  try {
    const res = await axios.post("http://localhost:8000/api/updatepassword", {
